...
function getFieldLabel(element: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
  ...
  const label = labelElement
    ? (() => {
      const labelCopy = labelElement.cloneNode(true) as HTMLLabelElement;
      labelCopy.querySelectorAll(
        'input, select, textarea, [data-inline-validation-error]'
...
function getValidationMessage(
  ...
  if (validity.tooShort) {
    return `Add at least ${element.getAttribute('minlength')} characters to ${label}.`;
  }
  if (validity.tooLong) {
    return `${label} cannot contain more than ${element.getAttribute('maxlength')} characters.`;
  }
...
export default function ApiErrorToast() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(eventName, handleError);
    return () => window.removeEventListener(eventName, handleError);
  }, []);

  useEffect(() => {
    document.querySelectorAll('[data-inline-validation-error]')
